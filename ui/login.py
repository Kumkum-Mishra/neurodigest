...
# Robust import: try absolute import first, then fall back to adding the package
# root to `sys.path` so the import works when Streamlit runs this file as a script.
import os
import sys
try:
    from ui.ui_helpers import inject_css, hero_banner
except Exception:
...
# Use only the hero banner for the main title to avoid duplication
# (hero_banner below renders the project title and CTA)

col1, col2, col3 = st.columns([1, 1, 1])
with col2:
        # use the hero helper for a more attractive CTA
        try:
                hero_banner(
                    "NeuroDigest — Personalized Content Digest & Learning Assistant",
                    "Curated articles, practice questions, and role-based learning roadmaps. Build practical skills for interviews, DSA, and AI careers.",
                    cta="Get Started",
                )
        except Exception:
                st.markdown("""
...
if not st.session_state.token:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        if me.status_code == 200:
                            ...
                            # set a short-lived flash message (auto-hides after 2s)
                            st.session_state._login_flash_ts = int(time.time())
                            st.session_state._login_flash_msg = f"Logged in as {email}"
                            # automatically navigate to Dashboard by updating query params
                            try:
                                st.experimental_set_query_params(page="Dashboard")
                            except Exception:
                                # fallback: set a session flag the Dashboard page can read
    ...
    # Show a short-lived flash message after login (2s), then hide it
    now_ts = int(time.time())
    flash_ts = st.session_state.get("_login_flash_ts")
    flash_msg = st.session_state.get("_login_flash_msg")
    if flash_ts and flash_msg and now_ts - int(flash_ts) < 2:
        st.success(flash_msg)
    else:
        # no persistent success banner — keep the header minimal
    ...
    # navigate. Show a persistent logged-in banner and let the user use the
    # sidebar to navigate to Dashboard when they're ready.
    if email:
        st.success(f"Logged in as {email}")
        # small prompt as requested
        st.markdown("<div style='margin-top:6px; font-weight:700; color:#bfe8c9'>Move to Dashboard</div>", unsafe_allow_html=True)
    else:
