...
def hero_banner(title: str, subtitle: str = "", cta: str | None = None):
    ...
    This renders a full-bleed hero bar (stretches to viewport width) while
    keeping an inner constraint so the text doesn't become too wide on large
    screens.
    """
    cta_html = f'<a class="cta" href="#">{cta}</a>' if cta else ""
    html = (
        '<div class="nd-hero glow">'
